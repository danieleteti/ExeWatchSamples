{ *******************************************************************************
  ExeWatch FMX Sample Application

  This demo shows how to use every feature of the ExeWatch Delphi SDK
  in a FireMonkey application. It is the FMX equivalent of the VCL sample.

  FMX vs VCL — what changes for ExeWatch integration:
    - Use ExeWatchSDKv1.FMX instead of ExeWatchSDKv1.VCL in your uses clause.
      This hooks into FMX's Application.OnException to capture GUI exceptions
      that bypass System.ExceptProc.
    - All other SDK calls (logging, timing, breadcrumbs, metrics, etc.) are
      exactly the same — the SDK is framework-agnostic.
    - FMX apps can target Windows, macOS, Linux, iOS, and Android.
      The SDK automatically detects the platform and collects the appropriate
      device info (OS, hardware, screen, etc.).
...
uses
  ...
  FMX.Dialogs, FMX.StdCtrls, FMX.Controls.Presentation, FMX.Memo,
...
{ Returns the current process working set (physical memory) in megabytes.
  ...
  Windows-only — on other platforms returns 0 (replace with platform API). }
...
{$ELSE}
begin
  Result := 0; // TODO: implement for macOS/Linux/mobile
end;
{$ENDIF}

{ Returns the free disk space on drive C: in gigabytes (Windows) or root (other).
  Used by the periodic gauge demo to monitor available storage.
  DiskFree(3) returns free bytes for drive C: (1=A, 2=B, 3=C, ...).
  DiskFree(0) returns free bytes for the current drive. }
...
begin
  ...
  {$ELSE}
  FreeBytes := DiskFree(0);  // 0 = current drive / root
  {$ENDIF}
  if FreeBytes >= 0 then
...
begin
  ...
  EW.SetCustomDeviceInfo('sample', 'DelphiFMX');
